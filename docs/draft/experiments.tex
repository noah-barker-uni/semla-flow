...
\hline
$\|\bar{P}z\| \,/\, \|z\|$              & 0.043 & 0.091 & 0.200 & 0.328 & 0.622 & 0.868 & 0.987 & 0.994 \\
Effective atoms averaged, $n_{\text{eff}}$ & 17.3 & 17.0 & 15.8 & 13.5 & 7.5 & 3.1 & 1.25 & 1.14 \\
Normalised plan entropy                 & 0.999 & 0.996 & 0.982 & 0.951 & 0.804 & 0.504 & 0.128 & 0.079 \\
\hline
\end{tabular}
\caption{The Sinkhorn target measured on real QM9 molecules, averaged over $64$ molecules and $64$
noise draws. The first row is the mean per-atom distance of the target from the origin divided by
the same quantity for the unmodified data molecule. The second is
$n_{\text{eff}} = 1 / \sum_j \bar{P}_{ij}^2$, the effective number of data atoms contributing to
each row of the target: it equals $1$ for a one-hot row, $n$ for a uniform row, and $k$ for a row
spread evenly over $k$ atoms. Equivalently it is the effective sample size of the weighted average,
since averaging independent quantities with weights $p$ scales their variance by $\sum_j p_j^2$.
The third row is the Shannon entropy of a row of $\bar{P}$ divided by $\log N$. For the Hungarian
and Metropolis arms the first row is exactly $1$ and $n_{\text{eff}}$ exactly $1$ at every $t$,
since both apply a genuine permutation.}
...
At $t = 0.05$ the target for each atom is an average of more than seventeen of the eighteen atoms in
a typical QM9 molecule, which is the molecular centroid; and since these molecules are centred at
the origin, that centroid \emph{is} the origin. Over the low-$t$ half of every trajectory the model
