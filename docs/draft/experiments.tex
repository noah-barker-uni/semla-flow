...
%% ==== PENDING: the Sinkhorn arm is being rerun (bond-label defect, job 6218549). ====
%% Non-bonded pairs were stored as ALL-ZERO vectors rather than one-hot on 'no bond', so the
%% soft path gave that class zero loss and zero gradient and the model generated complete
%% graphs. The hard arms argmax the target and are unaffected. DO NOT SUBMIT the claim below
%% that soft categorical targets destroy the bond graph until the rerun lands.
